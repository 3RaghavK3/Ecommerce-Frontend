...
export function FilterColumn() {
  ...
  const { categoryItems, setCategoryItems,open,setOpen } = useContext(MarketContext);
  useEffect(() => {
  ...
  return (
    <div className={`h-max absolute z-10 transition-transform ease-in-out lg:opacity-100 lg:translate-x-0 lg:pointer-events-auto  lg:relative  lg:w-1/4  text-sm md:text-base 
      ${
      open
      ...
      <CardHeader className="w-full p-0">
        <CardTitle className="flex flex-row justify-between font-semibold items-center">
          ...
          <div className="lg:hidden" onClick={()=>setOpen(false)}><X/></div>
          </div>
