    ...
    const multiplemarket=useRef([]);
      ...
      try {
        ...
        for (let index = 0; index < categoryItems.length; index++) {
          ...
          const response = await fetch(`http://localhost:3000/filter/categories?category=${category}`);
          const res = await response.json();
          console.log(market)
          
          
          res.data.forEach((product)=>multiplemarket.current.push(product))
          settotalproducts((prev) => (index === 0 ? res.total : prev + res.total));

          totalsum += res.total;
        }
        console.log(multiplemarket);
        totalPages.current = Math.ceil(totalsum / limit);
        setmarket(multiplemarket.current.slice(limit * (page - 1), (limit * (page - 1)) + limit))
      ...
      useEffect(() => {
        if(categoryItems.length===0 || categoryItems.length===1){
            let url;
            if(categoryItems.length===1) url=`http://localhost:3000/filter/category?category=${categoryItems[0]}&page=${page}&sortstate=${sort}`;
            else url=`http://localhost:3000/products?page=${page}&sortstate=${sort}`
            fetch(url)
            .then((res) => res.json())
          .then((x) => {
            setmarket(x.data);
      ...
      useEffect(()=>{if(categoryItems.length>1)repeatedFetch()},[categoryItems.length])
